...
Document de suivi, pas une spec. Sert de point d'entrée pour reprendre le travail dans une nouvelle session/IA sans avoir à relire toute la conversation d'origine. Mis à jour au fil des phases — dernière mise à jour : 2026-07-12, stabilisation post-Phase 4 (v3.0.1, voir §4.13/§4.14).
...
| Phase 3 — agent orchestrateur `ask` | ✅ Mergée, révisée en 3.0.1 | [design](specs/2026-07-09-harness-phase3-ask-orchestrator-design.md) | [plan](plans/2026-07-09-harness-phase3-ask-orchestrator.md) | `build_ask_options()` (routeur + 2 sous-agents à l'origine, repassé en agent racine unique en 3.0.1 pour fiabilité — voir §4.14) ; `/ask`/`explain_anomaly` migrés |
...
agent/harness/
  options.py  + build_ask_options(tenant_id, system_prompt=None) — Phase 3, RÉVISÉ (voir §4.14).
              La conception « routeur pur + 2 sous-agents » a été abandonnée : un appel d'outil
              MCP fait depuis un sous-agent et gardé par un hook PreToolUse échoue de façon
              intermittente (« Stream closed », ~4 échecs / 6 runs réels), alors que le même
              hook sur un appel fait par l'agent racine n'a jamais échoué (0/4). build_ask_options
              est désormais un AGENT RACINE UNIQUE doté des 5 outils (vigie-obs + vigie-biz),
              sans agents={}, avec hooks PreToolUse + PostToolUse complets.
...
9. **`ClaudeAgentOptions.disallowed_tools` (racine, blocklist) désactive aussi silencieusement l'outil pour un sous-agent qui l'a pourtant dans son propre `AgentDefinition.tools`** — testé isolément (claude_agent_sdk 0.2.113 / CLI 2.1.202) : le sous-agent ne reçoit aucune erreur, il invente une réponse plausible (0 appel outil réel, cf. `tool_uses` du résumé de tâche). `ClaudeAgentOptions.allowed_tools` (allowlist) au contraire ne restreint que le root — le sous-agent garde un accès normal à ses propres outils MCP in-process, tool_uses réel confirmé. Un sous-agent PEUT invoquer un serveur MCP in-process, et un hook PreToolUse (deny, et allow+updatedInput) s'y déclenche bien, **en isolation** (tests ping/pong ci-dessous, 100% de succès). Ce fait corrige l'ancienne conclusion « disallowed_tools racine et AgentDefinition.tools sont indépendants » (tirée d'un run Phase 3 antérieur) sans pour autant valider le design routeur — voir §4.14 pour la raison réelle de son abandon (fiabilité, pas impossibilité).
...
13. **Le CLI Claude Code refuse `--dangerously-skip-permissions` (⇐ `permission_mode="bypassPermissions"`) en root** : message « --dangerously-skip-permissions cannot be used with root/sudo privileges for security reasons », sortie code 1, remontée par le harness comme « Erreur harness agentique : Command failed with exit code 1 ». Le conteneur agent tournant en root, il faut poser `IS_SANDBOX=1` (échappatoire officielle pour environnement conteneurisé) — fait dans `agent/Dockerfile` + `docker-compose.yml`. Confirmé par un run réel non mocké.
14. **Sous-agent + hook `PreToolUse` + outil MCP réel (I/O réseau/DB) = échec intermittent « Stream closed »**, sur le CLI bundlé actuel (claude_agent_sdk 0.2.113 / CLI 2.1.202). Contrairement à ce qu'un premier diagnostic avait conclu (voir historique git), ce n'est **ni** une impossibilité structurelle pour un sous-agent d'invoquer un serveur MCP in-process, **ni** un canal `can_use_tool` réservé à la racine : un test isolé (outil `ping` factice, un seul hook léger) réussit à 100% avec sous-agent + hook (deny et allow+updatedInput) + MCP in-process. C'est en insérant les VRAIS hooks de production (`make_budget_guard_hook` + `make_tenant_scope_hook`, qui ouvrent une session SQLite synchrone) et les VRAIS outils (`run_query_loki`/`run_query_prometheus`, appels réseau réels) via `build_ask_options` complet que le run réel (`run_agent("ask", ...)`, tenant réel, Loki/Prometheus réels dans docker-compose) échoue à 4 reprises sur 6, avec le message JS `Error in hook callback ... error: Stream closed` (rejet de `sendRequest` / `Tool permission stream closed before response received`) — une race dans le canal de contrôle du CLI bundlé (binaire Bun, pas de source lisible), pas isolée plus précisément faute d'accès aux sources. Le même hook sur un appel outil fait directement par l'agent racine (`build_diagnostic_options`, mêmes outils réels) n'a **jamais** échoué sur 4 runs. Conséquence pratique : `build_ask_options` reste un agent racine unique (voir §3 ci-dessus) — pas par nécessité technique absolue, mais parce que la combinaison sous-agent+hook+outil réel n'est pas fiable en pratique sur ce CLI. Indépendamment de cette question, le scope tenant LogQL est déplacé du hook vers la couche outil (`agent/tools/loki.py::_scope_logql_to_tenant`) car deux chemins réels contournent totalement les hooks SDK : le serveur MCP externe (`agent/mcp/tools.py`, Phase 4) et `agent/services/alerting.py`, qui appellent `run_query_loki` directement sans jamais passer par `ClaudeAgentOptions.hooks`.
